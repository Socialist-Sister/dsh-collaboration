...
export class GeminiAdapter extends LlmAdapter {
  ...
  async *stream(options: GenerateOptions): AsyncGenerator<StreamChunk> {
    ...
    try {
      ...
      if (timeoutOf(watchdog.signal, STREAM_IDLE_TIMEOUT_CODE) !== undefined) {
        throw new LlmError(`Gemini stream idle timeout`, 'TIMEOUT', { cause: error })
      }
      if (options.signal?.aborted) throw new LlmError('request aborted by caller', 'ABORTED', { cause: error })
      if (error instanceof LlmError) throw error
      throw new LlmError(`Gemini API stream from ${connection.baseURL} failed`, 'TRANSPORT', { cause: error })
    } finally {
  ...
  private async *request(
    ...
    const endpoint = `${connection.baseURL}/v1beta/models/${encodeURIComponent(options.model)}:streamGenerateContent?alt=sse`
    ...
    try {
      response = await fetch(endpoint, { method: 'POST', headers, body: payload, signal })
    } catch (error) {
      if (signal.aborted) throw error
      throw new LlmError(`Gemini API request to ${connection.baseURL} failed`, 'TRANSPORT', { cause: error })
    }
    if (!response.ok) {
    ...
    if (!response.body) throw new LlmError('Gemini API returned no response body', 'EMPTY_RESPONSE')
